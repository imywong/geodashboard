...
const generateFaultCountByDayChart = (fault_date_counts) => {
    console.log("[generateFaultCountByDayChart] " + fault_date_counts)
    let faultDateCountLabels = Object.keys(fault_date_counts)
    let faultDateCountDataValues = Object.values(fault_date_counts)

    // //This converts the faultDateCountDataValues array to {x:, y: } format, but its not necessary
    // let faultDateCountDataValues = []
    // faultDateCountLabels.forEach((dateRecord)=>{
    //     faultDateCountDataValues.push({x:dateRecord,y:fault_date_counts[dateRecord]})
    //     // faultDateCountDataValues.push({x:'yeah'})
    // })
